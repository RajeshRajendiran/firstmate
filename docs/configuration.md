...
`send` renders replies as Telegram HTML (only `**bold**` and `` `code` `` become markup; all other text is escaped literally), splits them at 4,096 characters on line boundaries with any open tag closed and reopened across the split so every chunk is well formed, resends a chunk unformatted and prints a `fallback:` line if Telegram rejects its markup, respects the one-message-per-second limit, reports `delivered`, `ambiguous`, or `not-delivered` for each chunk, and stops at the first chunk that is not delivered and exits nonzero.
