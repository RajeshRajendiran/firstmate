...
`bin/fm-lint.sh` owns two canonical CI partitions, each running the same full source-aware ShellCheck analysis with up to four bounded workers, analyzing each root against a private view that includes every library once, pinned versions, workflow validation, and backend-purity checks.
