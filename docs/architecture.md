...
No-verb wakes, such as `working:` notes and bare turn-ended signals, are benign only when `bin/fm-crew-state.sh` reports positive evidence that the crew is still working: a currently attributed active no-mistakes run, or an exact busy verdict from the semantic busy-state contract.
...
Only the crew branch's newest run is ever a candidate: a newer run supersedes every older one on that branch, so a newest run that fails the identity check leaves the crew with no attributed run instead of reviving an older, superseded verdict.
