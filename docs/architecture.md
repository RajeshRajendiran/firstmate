...
A pane whose own attributed no-mistakes run shows positive progress - a `last_activity` stamp newer than its quiet window or a live `agent_pid`, both read through `bin/fm-crew-state.sh`'s probe mode - is likewise deferred instead of escalated, on the same bounded cadence and with the escalation counter untouched.
Every absence of progress evidence, including no attributable run, coarse-only run attribution, a parked or terminal run, and stale or unparseable activity, leaves the existing escalation schedule untouched, so an idle pane with no run behind it still escalates exactly as before.
