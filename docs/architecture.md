...
Actionable wakes include captain-relevant status signals, no-verb signals whose crew is not provably working, authenticated check output such as PR merge polling or a Relay mention, stale panes whose crew is not provably working whether their status log looks terminal or non-terminal, provably-working stale panes that persist past `FM_STALE_ESCALATE_SECS` without either task-worktree write evidence or positive progress from their own attributed pipeline run, declared external waits and verified captain-held transfers that remain declared past `FM_PAUSE_RESURFACE_SECS`, and heartbeat backstop hits.
...
Every absence of write evidence, including a missing worktree record, a torn-down worktree, a walk that outlives its wall-clock bound on a hung mount, and a failed walk, leaves the existing escalation schedule untouched - the write probe never suppresses an escalation on its own.
A secondmate's recorded worktree is never probed for write activity, because it is a provisioned firstmate home whose own supervision keeps writing inside it whether or not the mate produces anything; the write probe therefore never suppresses an escalation for a mate on its own.
A pane whose own attributed no-mistakes run shows positive progress - a `last_activity` stamp newer than its quiet window, an `active_for` showing the active step itself started inside that window, or a live `agent_pid`, all read through `bin/fm-crew-state.sh`'s probe mode - is likewise deferred instead of escalated, on the same bounded cadence and with the escalation counter untouched.
Every absence of progress evidence, including no attributable run, coarse-only run attribution, a parked or terminal run, and a step that has been running since before the quiet window with stale or unparseable activity and no live agent, leaves the existing escalation schedule untouched, so an idle pane with no progressing run behind it still escalates exactly as before.
A busy pane is otherwise exempt from staleness, but only until its latest `state/<id>.turn-ended` marker reaches `FM_BUSY_TURN_MAX_SECS`, or its `state/<id>.meta` spawn record reaches that age before any turn completes; past that bound it is routed through the same wedge escalation, with the identical reason, escalation count, write/progress deferrals, and `demand-deep-inspection` marker, for inspection only - never an automatic interrupt, signal, or restart.
...
Away-mode housekeeping has neither a worktree-write nor a pipeline-progress deferral of its own, so while `state/.afk` exists a quiet crew still escalates as a possible wedge at that bound even while it is writing its own worktree or its pipeline is still progressing.
