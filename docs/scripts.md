...
| `fm-telegram.sh`         | Telegram plane: `poll` long-polls the captain's private chat and surfaces each accepted message as a `check` wake, `listen` loops `poll` for near-instant delivery, `respond` sends one durable acknowledgement without starting work, `send` replies in 4,096-character chunks, `status` reports configuration and offset (schema: docs/configuration.md "Telegram plane") |
