...
#      Only the branch's NEWEST run is ever a candidate: a newer run supersedes
#      every older one on that branch, so a head mismatch on the newest run
#      attributes NOTHING rather than falling back to an older row (see
#      nm_runs_status_for_branch). The guarantee that buys is bounded and worth
#      stating exactly: the run-step path never reports a superseded run's
#      verdict as current. It is NOT a guarantee that no stale terminal verdict
#      can surface at all. With no attributable run this drops to step 4, and
#      there an earlier run's `failed:`/`done:` status-log line still becomes
#      `state: failed|done - source: status-log` whenever the pane reads idle -
#      which it does while the pipeline daemon, not the crew, owns the fix
#      round. KNOWN LIMITATION, deferred: a live (pending/running) newest row is
#      positive proof the crew is not terminal, so that case could suppress the
#      terminal log fallback instead; today the lookup reports only
#      "attributable or not" and cannot tell step 4 the difference.
