...
- `telegram-captain-channel` - load on a `check: telegram <update_id>` wake to inspect the background response record, avoid duplicate replies, and handle the message through the Telegram plane when needed.
